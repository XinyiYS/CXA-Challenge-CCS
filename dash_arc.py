...
def run(C_CONN):    
    ...
    ################################################################################
    # P_CONN,C_CONN = Pipe()
    # P_CONN.send(pickle.dumps(['hr','apple']))
    # P_CONN.send(pickle.dumps(['op','google']))
    # P_CONN.send(pickle.dumps(['fin','amazon']))
    # P_CONN.send(pickle.dumps(['op','google']))
    ...
    try:
        ...
        pass
    model.predict(np.arange(1000).reshape(1,1000))
    with open('tweet_hist.txt','r') as r:
        tweet_hist = r.readlines()
            ...
            children = [dcc.Graph(id='hr',className='three columns',
                        ...
                        html.Div(id='box',className='three columns',
                            children=[       
                                    ...
                                    html.Div(id='twitter',style={'color': 'darkgrey', 'fontSize': 17})]
                                    ),],
                ...
                html.Div(id='second-row-second-col',
                    ...
                    children=[
                        html.Div(
                            children=[dcc.Graph(
                                id='center',
                                figure= update_fin (showlegend=True),
                            ), html.Div(id= 'pred', style={'color': 'black', 'fontSize': 17} )]
                        ),
                    dcc.Dropdown(
                            id='selector',
                            options=[
                    ...
                    html.Div(id='controller',style={'display':'none'}),
                    dcc.Interval(id='listener', interval= 8000, n_intervals=0),
                    ]
                ),
            ]
        ),
    ...
    @app.callback(Output('controller','children'),
        [Input('listener','n_intervals')],
        [State('controller','children')])
    def update_controller(interval,command_dict):
        args = pickle.loads(C_CONN.recv())
        commands = {}
        command,company = args[0],args[1]

        if command_dict is None:
            if command is None and company is None:
                command_dict = {'command':'op','company':'google'}
                return command_dict
            commands['command']= command if command else 'op'
            commands['company']= company if company else 'google'
        else:
            command_dict = json.loads(command_dict)

            commands['command']= command if command else command_dict['command']
            commands['company']= company if company else command_dict['company']

        # commands = {'command':'op','company':'apple'}
        return json.dumps(commands)

    @app.callback(Output('company-dropdown','value'),
        [Input('controller','children')],
        [State('company-dropdown','value')])
    def update_company(command_dict,value):
        if command_dict is None:
            return value
        command_dict = json.loads(command_dict)
        return command_dict['company']


    @app.callback(Output('selector','value'),
        [Input('controller','children')],
        [State('selector','value')])
    def update_selector(command_dict,value):
        if command_dict is None:
            return value
        command_dict = json.loads(command_dict)
    ...
    def gen_tweet(interval,company,tweets_dict):
        ...
        tweets_dict['sentiment'] += np.dot(influences,(predict.predict(model,texts)))
        tweets_dict['count'] += 5
    ...
    def update_pred(interval,company,tweets_dict):
        ...
        size = int(0.2*len(tweet_hist))
        tweets = random.choice(tweet_hist,size=size)
        sentiments = predict.predict(model,tweets)
        sentiment = np.sum(     sentiments/ np.linalg.norm(sentiments)  )
        total =    tweets_dict['count'] + size   
        tweet_performance= tweets_dict['sentiment'] * tweets_dict['count'] / (total) + sentiment* size / total
        
        tweet_performance = "+{:.2%}".format(tweet_performance) if tweet_performance>=0 else "{:.2%}".format(tweet_performance)
...
P_CONN.send(pickle.dumps(['fin','amazon']))
P_CONN.send(pickle.dumps(['op','google']))
P_CONN.send(pickle.dumps(['op','apple']))
P_CONN.send(pickle.dumps(['hr','facebook']))
P_CONN.send(pickle.dumps(['fin','google']))
